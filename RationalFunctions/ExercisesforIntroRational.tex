...
\item
\label{findformulafor1overxgraphfirst}  $y=F(x)$ in Figure \ref{fig:findformulafor1overxgraphfirst}
...
\caption{}
\label{fig:findformulafor1overxgraphfirst}
\end{mfigure}
    
\item 
\label{findformulafor1overxgraphlast}
$y = F(x)$ in Figure \ref{fig:findformulafor1overxgraphlast}
...
\caption{}
\label{fig:findformulafor1overxgraphlast}
\end{ifigure}
...
\item $~$ \label{findformulafor1overxsquaredgraphfirst}
$y = F(x)$ in \autoref{fig:findformulafor1overxsquaredgraphfirst}%$F(x) = -\dfrac{4}{(x+2)^2}+4$
...
\caption{}
\label{fig:findformulafor1overxsquaredgraphfirst}
\end{ifigure}

\item $~$  \label{findformula1overxsquaredgraphlast}
$y = F(x)$ in \autoref{fig:findformula1overxsquaredgraphlast} %$F(x) = \dfrac{4}{(2x-1)^2}-4$
...
\sidenote{The authors wish to thank Don Anthan and Ken White of Lakeland Community College for devising this problem and generating the accompanying data set.}
...
\sidenote{See Definition \ref{arc} in Section \ref{AverageRateofChange} for a review of this concept, as needed.}  
...
Horizontal asymptote: $y= 2$\\ 
See \autoref{fig:fxeqfourxintotwoxplusoneetc}
...
No holes in the graph\\
Horizontal asymptote: $y = 0$ \\
\sidenote{This is hard to see on the calculator, but trust me, the graph is below the $t$-axis to the left of $t = -7$.}As $t \rightarrow -\infty, g(t) \rightarrow 0^{-}$\\
As $t \rightarrow \infty, g(t) \rightarrow 0^{+}$\\
...
\begin{mfigure}
\xincludegraphics{./IntroRationalGraphics/MaxPowerRegression.jpg}
