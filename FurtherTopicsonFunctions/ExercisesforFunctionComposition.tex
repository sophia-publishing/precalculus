...
\mexinstr{%
In Exercises \ref{pointcompexfirst} - \ref{pointcompexlast}, let $f$ be the function defined by 
\begin{multline*}
	f = \{(-3, 4), (-2, 2), \\
	(-1, 0), (0, 1), (1, 3),\\
	(2, 4),(3, -1)\}
...
\mexinstr{%
In Exercises \ref{twofuncgraphcompfirst} - \ref{twofuncgraphcomplast}, use the graphs of $y=f(x)$ and $y=g(x)$ in \autoref{fig:exfcompyeqfx} and \autoref{fig:exfcompyeqgx} to find the following if it exists.
}
...
\item $(f \circ g)(x) =    \begin{mycases} 6x-2 &  \text{if $x \leq 3$} \\   13-3x  & \text{if $x > 3$} \\  \end{mycases}$ and

$(g \circ f)(x) =    \begin{mycases} 6x+1 &  \text{if $x \leq \frac{2}{3}$} \\   3-3x  & \text{if $x > \frac{2}{3}$} \\  \end{mycases}$
