...
\begin{multline*}
	...
	= \begin{cases}
		2(6x-x^2) -1 &  \text{if $-1 \leq 6x-x^2  < 3$, } \\
		(6x-x^2)^2  & \text{if $6x-x^2 \geq 3$.}
	\end{cases}
\end{multline*}
...
\[
	(g \circ f)(x) =
		\begin{cases}
		    -2x^2+12x-1 \\
				\qquad \text{if } x \in [3- \sqrt{10}, 3 - \sqrt{6}) \\
				\qquad\qquad \cup (3+\sqrt{6}, 3+\sqrt{10}]\text{,}\\
		    x^4 - 12x^3 + +36x^2  \\
				\qquad \text{if $x \in [3-\sqrt{6}, 3+\sqrt{6}]$.} \\
		\end{cases}
\]

\item  Last but not least, we are tasked with representing $s \circ h$ as a set of ordered pairs.  Since $h$ is described by the discrete set of points
\[h = \{ (-3,1), (-2,6), (0,-2), (1,5), (3,-1)  \}\], we find $s \circ h$ point by point.  From the graph of $s$ in \autoref{fig:yeqstfcompex3} we construct \autoref{tab:yeqstfcompex3} to help us organize our work. 
...
\begin{multline*}
	(h \circ g \circ f)(t) = h(g(f(t))) = h(g\left(t^2\right)) \\
	= h\left(t^2+1\right)= \frac{2}{t^2+1} = G(x).
\end{multline*}
...
\item To write $F = f \circ \left(\frac{g}{h} \right)$ means
\begin{multline*}
F(x) =  \sqrt{\dfrac{2x-1}{x^2+4}} = \left( f \circ \left(\dfrac{g}{h} \right) \right)(x) \\
= f \left(  \left(\dfrac{g}{h} \right)(x) \right) = f \left( \dfrac{g(x)}{h(x)} \right).
\end{multline*}
...
\input{ExercisesforFunctionComposition}
